...
def  simulation(process_objs, verbose_flag, random_numb, input_file):
    ...
    FCFS = FCFS_sch(process_objs, random_numb, orig_line, verbose_flag)
...
def FCFS_sch(process_objs, random_numb_global_copy, orig_line, verbose_flag):
    ...
    output = output + 'The sorted input is: ' + procobjs2str(process_objs) + '\n'

    

    if (verbose_flag == True):
        output += 'Detailed printout\n\n'

    deep_procs_copy = copy.deepcopy(process_objs)
    output_cycles = []
    readyq = deque([])
    curr_proc = None
    ...
    curr_cycle = 0
    while(not all_procs_term(deep_procs_copy)):
        readyq_len = len(readyq)
        
        cycle_output = gen_cycle_output(deep_procs_copy, curr_cycle) + '\n'

        
        for idx, process in enumerate(deep_procs_copy):

            if (process['A'] == curr_cycle and process not in readyq and process['state'] == 'unstarted'):
            ...
            if (process['state'] == 'running'):
                ...
                if (process['remainCPUB'] == 0):
                    set_to_blocked = True

                    for p in deep_procs_copy:
                        if (p['state'] == 'blocked' and p['modified']):
                            set_to_blocked = False;
                            break
                        if (p['state'] == 'blocked' and not p['modified'] and p['remainingIOB'] != 1):
                            set_to_blocked = False;
                            break
                    ...
                    if (set_to_blocked == False):
                        ...
                        process['remainingIOB'] = IO_burst
                        continue


            if (process['state'] is 'blocked'):
                process['remainingIOB'] -= 1
                run_data['IO_util'] += 1
                proc_data[idx]['IO_time'] += 1
                process['modified'] = True

                if (process['remainingIOB'] is 0):
                    ...
                    if (len(readyq) != 0):
                        ...
                        for p in deep_procs_copy:
                            if (p['state'] is 'running' and p['modified']):
                                set_to_running = False;
                                break
                            if (p['state'] is 'running' and not p['modified'] and p['remainCPUB'] is not 1):
                                set_to_running = False;
                                break
    ...
    for data in proc_data:
        totwait += data['wait_time']
        totturn += (data['finish_time'] - data['A'])

    run_data['average_wait'] = str(float(totwait)/len(proc_data))
    run_data['average_turnaround'] = str(float(totturn)/len(proc_data))

    output += '\nThe scheduling algorithm: first come first served.\n\n'

    for idx, proc in enumerate(process_objs):
        output+= ('Process ' + str(idx) + ':\n')
        output+= ('\t(A,B,C,IO) = ' + process_object_to_string(proc) + '\n')
        output+= ('\tFinishing time: ' + str(proc_data[idx]['finish_time']) + '\n')
        output+= ('\tTurnaround time: ' + str(int(proc_data[idx]['finish_time']) - int(proc['A'])) + '\n')
        output+= ('\tI/O time: ' + str(proc_data[idx]['IO_time']) + '\n')
        output+= ('\tWaiting time: ' + str(proc_data[idx]['wait_time']) + '\n')
        output+= '\n'

    output = output + 'Summary Data:\n'
    output = output +  '\tFinishing time: ' + str(run_data['finish_time']-1) + '\n'
    output = output +  '\tCPU Utilization: ' + str( float(run_data['CPU_util']) / (run_data['finish_time']-1)) + '\n'
    output = output +  '\tI/O Utilization: ' + str( float(run_data['IO_util']) / (run_data['finish_time']-1)) + '\n'
    ...
    output = output +  '\tAverage turnaround time: ' + str(run_data['average_turnaround']) + '\n'
    output = output +  '\tAverage waiting time: ' + str(run_data['average_wait']) + '\n'
