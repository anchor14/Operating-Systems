...
def create_process_objs(file_data):
    ...
    for line in file_data:
        ...
        cleaned_processes = list(filter(lambda x: x != ' ', process_delin))


        if (len(cleaned_processes) % 4 != 0):
...
def  simulation(process_objs, verbose_flag, random_numb, input_file):
    ...
    RR = RR_sch(process_objs, random_numb, orig_line, verbose_flag)
...
def RR_sch(process_objs, random_numb_global_copy, orig_line, verbose_flag):
    ...
    while(not all_procs_term(deep_proc_copy)):
        ...
        for idx, process in enumerate(deep_proc_copy):
            ...
            if (process['state'] == 'running'):
                ...
                if (process['remainCPUB'] == 0 or process['quantum'] == 0):
                    set_to_blocked = True
    ...
    run_data['avgTurnaround'] = str(float(tot_turn)/len(proc_data))

    output += '\nThe scheduling algorithm: Round Robin\n\n'
