...
def  simulation(process_objs, verbose_flag, random_numb, input_file):
    ...
    SRTN = SRTN_sch(process_objs, random_numb, orig_line, verbose_flag)
...
def SRTN_sch(process_objs, random_numb_global_copy, orig_line, verbose_flag):
    ...
    while(not all_procs_term(deep_proc_copy)):
        ...
        for idx, process in enumerate(deep_proc_copy):
            ...
            if (process['A'] == curr_cycle and process not in readylst and process['state'] == 'unstarted'):


                if (curr_proc == None):
                    if (len(readylst) != 0):
                        process['state'] = 'ready'
                        readylst.append(process)
                        continue
            ...
            if (process['state'] == 'blocked'):
                ...
                if (process['remainIOB'] == 0):
                    ...
                    #             break
                    #         if (p['state'] == 'running' and not p['modified'] and p['remainCPUB'] != 1):
                    #             set_to_running = False;
                    #             break


                    #     if (set_to_running != True):
                    #         process['state'] = 'ready'
                    #         readylst.append(process)
                    #         continue
                    ...
                    #     sht_proc = findshtproc(process, readylst)
            ...
            if (process['state'] == 'ready'):
                ...
                if (process != sht_proc):
                    ...
                    if (set_to_running == False):
                        ...
                        readylst.pop(readylst.index(process))
                        process['state'] = 'running'
                        CPU_burst = randomOS(process['B'], random_numb)
    ...
    for data_obj in proc_data:
        totwait += data_obj['wait_time']
        totturn += (data_obj['finish_time'] - data_obj['A'])

    run_data['avgWait'] = str(float(totwait)/len(proc_data))
    run_data['avgTurnaround'] = str(float(totturn)/len(proc_data))

    output += '\nThe scheduling algorithm: SRTN \n\n'
    ...
    output = output +  '\tThroughput: ' + str(throughput) + ' per hundred cycles' + '\n'

    output+= '\tAverage turnaround time: ' + str(run_data['avgTurnaround']) + '\n'
    output+= '\tAverage waiting time: ' + str(run_data['avgWait']) + '\n'
...
def findshtproc(curr_proc, readylst):
    run_min = curr_proc
    for process in readylst:
        if (process['state'] == 'unstarted' or process['state'] == 'terminated'):
            continue

        if ((process['remainCPUB'] > 0 and (process['remainCPUB'] < run_min['remainCPUB'])) or (process['C'] < run_min['C'])):
                run_min = process
